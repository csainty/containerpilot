...
type LeaseInfo struct {
	...
	DurationInSecs         uint `xml:"durationInSecs,omitempty" json:"durationInSecs,omitempty"`
...
func (c *Client) getCancelable(endpoint string,
	...
	log.Debug("get %s [%s]", endpoint, c.Cluster.Leader)
	p := endpoint
...
func (c *Client) Put(endpoint string, body []byte) (*RawResponse, error) {
	...
	log.Debug("put %s, %s, [%s]", endpoint, body, c.Cluster.Leader)
	p := endpoint
...
func (c *Client) Post(endpoint string, body []byte) (*RawResponse, error) {
	log.Debug("post %s, %s, [%s]", endpoint, body, c.Cluster.Leader)
	p := endpoint
...
func (c *Client) Delete(endpoint string) (*RawResponse, error) {
	log.Debug("delete %s [%s]", endpoint, c.Cluster.Leader)
	p := endpoint
...
func (c *Client) SendRequest(rr *RawRequest) (*RawResponse, error) {
	...
	if rr.cancel != nil {
		...
		go func() {
			...
			case <-rr.cancel:
				cancelled <- true
				log.Debug("send.request is cancelled")
			case <-cancelRoutine:
				return
			}
	...
	for attempt := 0; ; attempt++ {
		...
		log.Debug("Connecting to eureka: attempt %d for %s", attempt+1, rr.relativePath)

		httpPath = c.getHttpPath(false, rr.relativePath)

		log.Debug("send.request.to %s | method %s", httpPath, rr.method)
		...
		if err != nil {
			log.Error("network error: %v", err.Error())
		...
		log.Debug("recv.response.from " + httpPath)

		if validHttpStatusCode[resp.StatusCode] {
			// try to read byte code and break the loop
			respBody, err = ioutil.ReadAll(resp.Body)
			if err == nil {
				log.Debug("recv.success " + httpPath)
				break
			}
		...
		if resp.StatusCode == http.StatusTemporaryRedirect {
			...
			if err != nil {
				...
				log.Debug("recv.response.relocate " + u.String())
			}
			resp.Body.Close()
			continue
		}
...
func DefaultCheckRetry(cluster *Cluster, numReqs int, lastResp http.Response,
	...
	log.Warning("bad response status code %d", code)
	return nil
}
